...
#include "hbk/communication/wmi.h"
...
namespace hbk {
	namespace communication {
		...
		IWbemLocator*	WMI::m_pLoc = nullptr;
		IWbemServices*	WMI::m_pSvc = nullptr;
		...
		bool WMI::isFirewireAdapter(const communication::Netadapter &adapter)
		{
			...
			IWbemClassObject *pWMIAdapter = nullptr;
			unsigned long ret;
		...
		long WMI::setManualIpV4(const communication::Netadapter &adapter, const communication::Ipv4Address &manualConfig)
		{
			...
			hres = m_pSvc->GetObject(L"Win32_NetworkAdapterConfiguration", 0, nullptr, &pClass, nullptr);
			if (WBEM_S_NO_ERROR != hres) {
				return -1;
			}

			// Get the EnableStatic method informations using the class pointer we just retrieved
			IWbemClassObject *pParamsIn = nullptr;
			hres = pClass->GetMethod(L"EnableStatic", 0, &pParamsIn, nullptr);
			pClass->Release();
			if (WBEM_S_NO_ERROR != hres) {
				return -1;
			}
